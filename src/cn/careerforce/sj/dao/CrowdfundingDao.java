...
public class CrowdfundingDao {
    ...
    public List<Map<String, Object>> queryCFDetailsByCFId(String cfId) {
        String sql = "SELECT id, `name`, price, number_limit_type, number_limit_count, supported_number, content, description, 1 AS seq FROM crowdfunding_detail WHERE price > 0 AND number_limit_type = 0 AND crowdfunding_id = " + cfId + " UNION ALL SELECT id, `name`, price, number_limit_type, number_limit_count, supported_number, content, description, 1 AS seq FROM crowdfunding_detail WHERE price > 0 AND number_limit_type = 1 AND number_limit_count > supported_number AND crowdfunding_id = " + cfId + " UNION ALL SELECT id, `name`, price, number_limit_type, number_limit_count, supported_number, content, description, 2 AS seq FROM crowdfunding_detail WHERE price < 0 AND crowdfunding_id = " + cfId + " UNION ALL SELECT id, `name`, price, number_limit_type, number_limit_count, supported_number, content, description, 3 AS seq FROM crowdfunding_detail WHERE price > 0 AND number_limit_type = 1 AND number_limit_count <= supported_number AND crowdfunding_id = " + cfId + " ORDER BY seq, price";
        return jdbcTemplate.queryForList(sql);
    }
