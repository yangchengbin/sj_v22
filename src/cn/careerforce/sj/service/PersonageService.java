...
public class PersonageService {
    ...
    public List<Map<String, Object>> queryPersonsWithKey(String type, String key, int recordIndex, int pageSize) {
        ...
        if ("0".equals(type)) {
            ...
            sql = "SELECT p.id, p.user_id AS userid, p.pname AS title, p.career AS summary, p.head_img AS pictureUrl FROM personage p WHERE p.valid = 1 AND p.pname LIKE '%" + key + "%' LIMIT " + recordIndex + ", " + pageSize;
